...
namespace talkie
{
	...
	{
		if (success)
			client->write(comm::encode_tuple(comm::encoder() << cmd << true, tuple));
		else
	...
	DirectoryServer::DirectoryServer(boost::asio::io_service& io_service, uint16 port,
		...
		: TCPCommandServer<DirectoryConnection, int32>(io_service, port), servers(servers)
	{
		...
		//-- Setup storage service manager
		StorageServiceManager::Instance().Start(GetIOService());
	}
