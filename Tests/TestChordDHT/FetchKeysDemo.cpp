 ...
 *	-- ChordDHT Test demo --
 *
 *	Demo testing fetch keys functionality of newly
 *	arrived chord nodes.
 *
...
void RunFetchKeysDemo(void)
{
	PrintHorzBar();
	std::cout << "Starting FETCH KEYS demo" << std::endl;
	PrintHorzBar();
	...
	for (int i=1; i < NPEERS; ++i)
	{
		...
		if (i > 1)
			join_timers[i]->expires_at(join_timers[i-1]->expires_at() + boost::posix_time::seconds(7/*rand() % 5*/));
		else
	...
	PrintHorzBar();
	std::cout << "Ended FETCH KEYS demo" << std::endl;
	PrintHorzBar();
}
