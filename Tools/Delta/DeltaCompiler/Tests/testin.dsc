...
// Testing of list and iterators.
//
print("Testing of lists and iterators.\n----------------------------\n");

t = ["One", "two", "three", 56, (function() { return; }) ];
print("Table is:\n", t);
l = list();

for (x=0; x<tablength(t); x++)
	list_push_back(l, t[x]);

print("List is:\n");
for (listiter_setbegin(l,x=listiter()); not list_checkend(l,x); listiter_fwd(x))
	print(listiter_getval(x),"\n");

// Testing of vectors.
//
print("Testing of vectors.\n----------------------------\n");
v = vector(list_total(l));
for (i=word(0), listiter_setbegin(l, x=listiter()); not list_checkend(l,x); listiter_fwd(x))
	vector_setval(v, i++, listiter_getval(x));

for (i=vector_total(v); i; )
	print(vector_getval(v, --i),"\n");
