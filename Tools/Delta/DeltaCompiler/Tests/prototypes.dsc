...
sum1_n = [
	(method(n) { return (n ? n + lambda(n-1) : 0); })
][0];
print("sum(10)=", sum1_n(10), nl);

std = std_getapi();
print(std);
 
vm = std.vm.load("vmfuncs_funcs.dbc", "funcs");
std.misc.print(vm, nl);
...
try { 
	...
	trap e { std.misc.print(e, nl); }
}

assert vm == std.vm.get("funcs");
funcs = std.vm.funcs(vm);
assert funcs.f = std.vm.funcaddr(vm, "f");
funcs.f();

assert funcs.g = vmfuncaddr(vm, "g");
funcs.g();
std.vm.unload(vm);

function Vector3(x,y,z) {        // Vector3 object constructor
   return [
       { .x : x }, { .y  : y }, { .z  : z },

       { .display : (
           method () {
				print(lpar, @x, space, @y, space, @z, rpar, nl);
           }
       )},
...
// Here we check the support for type casting.
// Conversion may be dispatched on the lvalue type (second arg).
//
tc = [ 
	{ .n : 7 }, 
	{ "=()" : (function(lv, rv) { return rv.n * rv.n; })} 
];
x = tc;
print(x, nl);
...
function mixin_comp() {	// The composer as a generic function.
	...
	comp = [ 
		...
		{.new	: (method(){			// Object constructor for the composite class object
				...
				for (i=1; i <= @n; ++i) {	// Instatiate all base objects
					args = [];
...
function deferred_call(f) {
	...
	// Copy actual arguments, besides f itself (arguments[0]).
	
	for (	local args = [], local i = 1, local n = arguments.total;
			i < n;
...
print("Total std functions:", dump(std, "std"), nl);
