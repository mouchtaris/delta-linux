...
WX_FUNC_ARGRANGE_START(toolbookevent_construct, 0, 4, Nil)
	...
	DeltaWxToolbookEvent *evt = DNEWCLASS(DeltaWxToolbookEvent,
		(new wxToolbookEvent(commandType, id, nSel, nOldSel)));
	WX_SETOBJECT(ToolbookEvent, evt)
}

DLIB_FUNC_START(toolbookevent_destruct, 1, Nil)
	DLIB_WXDELETE(toolbookevent, ToolbookEvent, evt)
}

DLIB_FUNC_START(toolbookevent_getoldselection, 1, Nil)
	DLIB_WXGET_BASE(toolbookevent, ToolbookEvent, evt)
	WX_SETNUMBER(evt->GetOldSelection())
}

DLIB_FUNC_START(toolbookevent_getselection, 1, Nil)
	DLIB_WXGET_BASE(toolbookevent, ToolbookEvent, evt)
	WX_SETNUMBER(evt->GetSelection())
}

DLIB_FUNC_START(toolbookevent_setoldselection, 2, Nil)
	DLIB_WXGET_BASE(toolbookevent, ToolbookEvent, evt)
	WX_GETNUMBER(nOldSel)
	evt->SetOldSelection(nOldSel);
}

DLIB_FUNC_START(toolbookevent_setselection, 2, Nil)
	DLIB_WXGET_BASE(toolbookevent, ToolbookEvent, evt)
	WX_GETNUMBER(nSel)
