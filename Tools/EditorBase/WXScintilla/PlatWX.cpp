...
public:
	...
	virtual PRectangle GetScreenRect();
...
void ListBoxImpl::Create (Window &parent, int ctrlID, Point location_,
	...
	// AS: creation location was not recorded
	location = location_;	
...
// AS: we need somehow to know the screen rect
PRectangle ListBoxImpl::GetScreenRect() {
	return PRectangle(0, 0, wxSystemSettings::GetMetric(wxSYS_SCREEN_X), wxSystemSettings::GetMetric(wxSYS_SCREEN_Y));
}
...
PRectangle ListBoxImpl::GetDesiredRect() {
	...
	int reserved_w = aveCharWidth * 3 + GETLBW(id)->IconWidth() + wxSystemSettings::GetMetric(wxSYS_VSCROLL_X);
    maxw += reserved_w;
	
#if	0	// AS: disabling old max width for list boxes

	if (maxw > 350)
        maxw = 350;
#else
	int screen_w = wxSystemSettings::GetMetric(wxSYS_SCREEN_X) - 150;
	if (maxw > screen_w)
		maxw = screen_w;
#endif
...
#if 0	// AS: disabling the height calculation logic
...
#else
		int lines = (maxh > 250 ? 250 : maxh) / rect.GetHeight();
        maxh = lines * rect.GetHeight();
		
		if (maxh > (wxSystemSettings::GetMetric(wxSYS_SCREEN_Y) - location.y)) {
			lines = (wxSystemSettings::GetMetric(wxSYS_SCREEN_Y) - location.y) / rect.GetHeight();
			maxh = lines * rect.GetHeight();
		}
#endif
    }
...
void ListBoxImpl::RegisterImage(int type, const char *xpm_data) {
    ...
    wxBitmap bmp((char const* const*) xpm_data);
