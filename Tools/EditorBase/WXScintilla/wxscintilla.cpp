...
BEGIN_EVENT_TABLE(wxScintilla, wxControl)
    ...
    EVT_MENU					(wxID_ANY, wxScintilla::OnMenu)
...
// Define a marker from a bitmap
void wxScintilla::MarkerDefineBitmap (int markerNumber, char const* const* xpmImage) {
    SendMsg (SCI_MARKERDEFINEPIXMAP, markerNumber, (long)xpmImage);
}
...
// Register an image for use in autocompletion lists.
void wxScintilla::RegisterImage (int type, char const* const* xpmImage) {
	SendMsg(SCI_REGISTERIMAGE, type, (long)xpmImage);
}
...
// Set the data that will be available to the lexer (through Accessor::GetUserData())
void wxScintilla::SetLexerUserData (void* data) {
	SendMsg (SCI_SETLEXER_USERDATA, (long) data);
}
...
bool wxScintilla::LoadFile (const wxString& filename, bool deleteUndoHistory) {
...
void  wxScintilla::AddContextMenuAction (
		const std::string&	id, 
		void				(*action)(void*, int, int),
		bool				(*precond)(void*, int, int),
		void*				closure
	) {  m_swx->AddContextMenuAction(id, action, precond, closure); }
