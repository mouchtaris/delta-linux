...
bool ScintillaBase::Command(int cmdId) {

	//***********************************
	// (AS) For context menu user entries.
	...
	switch (cmdId) {
		...
		default:	return false;	//Notify caller that the event was not processed
	}
	return true;
}
...
void ScintillaBase::AutoCompleteStart(int lenEntered, const char *list) {
	...
	widthLB				= Platform::Minimum(rcList.right - rcList.left, rcClient.right - rcClient.left);

	// Make an allowance for large strings in list

	rcList.left = pt.x - ac.lb->CaretFromEdge();
	if ((rcList.left + widthLB) >= rcClient.right)
		rcList.left = rcClient.right - widthLB;
...
void ScintillaBase::AutoCompleteCancel() {
	if (listType > 0) {	//Send notification when cancelling user list
...
void ScintillaBase::AutoCompleteCharacterDeleted() {
	if (currentPos < ac.posStart - ac.startLen) {
		ac.Cancel();
	} else if (ac.cancelAtStartPos && (currentPos < ac.posStart)) {	// AS: must be before start pos not equal
		ac.Cancel();
	} else {
		AutoCompleteMoveToCurrentWord();
	}
...
void ScintillaBase::InsertUserActionsInContextMenu (int x, int y) {
	contextMenuX = x, contextMenuY = y;
	for (int n = contextUserActions.size(), i = 0; i < n; ++i)
		AddToPopUp(
			contextUserActions[i].first.c_str(), 
			idcmdUserDefined + i, 
			contextUserActions[i].second.Precond(x, y)
		);
	if (!contextUserActions.empty())
		AddToPopUp("");
}

void ScintillaBase::AddContextMenuAction (
		const std::string&	id, 
		void				(*action)(void*, int, int),
		bool				(*precond)(void*, int, int),
		void*				closure
	) {
	contextUserActions.push_back(
		ContextMenuUserEntry(id, ContextMenuActionEntry(action, precond, closure))
	);
}
...
void ScintillaBase::ContextMenu(Point pt) {
	if (displayPopupMenu) {
		...
		popup.CreatePopUp();
		InsertUserActionsInContextMenu(pt.x, pt.y);	// (AS) For context menu user entries.
...
void ScintillaBase::Colourise(int start, int end) {
	if (!performingStyle) {
		...
		styler.SetUserData(lexUserData);
...
sptr_t ScintillaBase::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case SCI_SETLEXER_USERDATA:
		SetLexerUserData(reinterpret_cast<void *>(wParam));
		break;
