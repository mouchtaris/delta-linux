...
static void DrawBox(Surface *surface, int centreX, int centreY, int armSize, ColourAllocated fore, ColourAllocated back) {
	...
	surface->RectangleDraw(rc, fore, back);
}
...
static void DrawPlus(Surface *surface, int centreX, int centreY, int armSize, ColourAllocated fore) {
	...
	surface->MoveTo(centreX - armSize, centreY);
	surface->LineTo(centreX + armSize, centreY);
...
void LineMarker::Draw(Surface *surface, PRectangle &rcWhole, Font &fontForCharacter) {
	...
	rc.right -= 3; // Leave 3 pixels space to the right
	rc.top++;
	rc.bottom--;
	...
	if (markType == SC_MARK_ROUNDRECT) {
		...
		// repl:
		surface->LineTo(centreX, rcWhole.bottom);
		surface->LineTo(rc.right - 1, rcWhole.bottom);
		...
		DrawPlus(surface, centreX, centreY, blobSize, fore.allocated);

		surface->MoveTo(centreX, centreY + blobSize);
		surface->LineTo(centreX, rcWhole.bottom - 1);
		surface->LineTo(rc.right, rcWhole.bottom - 1);
		...
		surface->MoveTo(centreX, rcWhole.bottom - 1);
		surface->LineTo(rc.right, rcWhole.bottom - 1);

	} else if (markType == SC_MARK_BOXMINUS) {
		surface->PenColour(fore.allocated);
		DrawBox(surface, centreX, centreY, blobSize, fore.allocated, back.allocated);
		DrawMinus(surface, centreX, centreY, blobSize, fore.allocated);
