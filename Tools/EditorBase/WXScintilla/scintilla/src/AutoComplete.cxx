...
AutoComplete::AutoComplete() :
	...
	// AS: extensions for artificial prefixes
	firstAppearence(true),
	isArtificialPrefix(false)	

{
	lb = ListBox::Allocate();
	stopChars[0] = '\0';
	fillUpChars[0] = '\0';
	activationPrefix[0] = '\0';
}
...
void AutoComplete::Select(const char *word) {
	// AS: extensions for artificial prefixes
	if (firstAppearence)	
		strcpy(activationPrefix, word);
	else
	if (isArtificialPrefix)
		if (strlen(word) <= strlen(activationPrefix))
			return;
		else
			word += strlen(activationPrefix);	// Ignore entirely the artifical prefix in matching items.
	...
	if (location == -1 && autoHide) {
		...
		// AS: extensions for artificial prefixes
		if (firstAppearence)
			isArtificialPrefix = true;
		if (!firstAppearence && strcmp(activationPrefix, word))
			Cancel();
	}
	else
		lb->Select(location);
